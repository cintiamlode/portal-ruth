...
app.get('/lancamentos/:tipo', async (req, res) => {
  try {
    ...
    if (modo === 'competencia') {
      ...
      // data_pagamento = Data movimento no extrato CA (FCX)
    ...
    const data = await r.json();
    const itens = data?.itens || data?.content || [];
    console.log('  ->', r.status, 'n=', itens.length);
    if(itens.length > 0) console.log('  item[0]:', JSON.stringify(itens[0]).substring(0,500));
